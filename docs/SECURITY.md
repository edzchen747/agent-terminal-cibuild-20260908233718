...
For cross-network use, deploy `server/relay`. Headscale coordinates the embedded nodes and its DERP/STUN service attempts a direct NAT-traversed path before encrypted fallback. Project state, terminal state, and device authorization remain on the desktop, which verifies the saved device credential after the overlay connection is established.
