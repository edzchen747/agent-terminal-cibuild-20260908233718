...
/** The blue of the app icon (the launcher foreground), the brand accent. */
const ICON_BLUE = "#5db8fd";

test("the settings sheet offers system, light and dark", () => {
...
test("system follows the phone setting and an explicit choice overrides it", () => {
...
test("the accent is the blue from the app icon, matching the desktop", () => {
  assert.match(styles, new RegExp(`--accent:\\s*${ICON_BLUE}`, "i"));
  // Drift guard: the previous teal accent must not come back anywhere.
  for (const hex of ["#7dd8c4", "#72d2bf", "#8be0ce", "#7dd5c3", "#83d8c6", "#8ce0cf"]) {
