...
export function App() {
  ...
  function finishSwipe(event: ReactPointerEvent<HTMLDivElement>, cancelled = false) {
    ...
    if (suppressSwipeClickRef.current) {
      // The compatibility click generated by this pointer-up is dispatched
      // before the next task. Never leave suppression armed for a later tap.
  ...
  return <div className="mobile-pager" onPointerDownCapture={() => { suppressSwipeClickRef.current = false; }} onPointerDown={beginSwipe} onPointerMove={moveSwipe} onPointerUp={(event) => finishSwipe(event)} onPointerCancel={(event) => finishSwipe(event, true)} onLostPointerCapture={(event) => finishSwipe(event, true)} onClickCapture={suppressSwipeClick}>
